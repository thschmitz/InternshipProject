...
public class Instantiation implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		Post post1 = new Post(null, sdf.parse("12/07/2022"), "Teste01", "Testando um novo post1", "https://www.pousadasincriveis.com/wp-content/uploads/2022/06/interiorsp-13.webp", "A venda", 100.0, 20.3,  new AuthorDTO(maria).getId());
		Post post2 = new Post(null, sdf.parse("23/10/2022"), "Teste02", "Testando um novo post2", "imagemNenhuma", "A venda", 130.0, 25.0, new AuthorDTO(alex).getId());
		Post post3 = new Post(null, sdf.parse("05/09/2022"), "Teste03", "Testando um novo post3", "imagemNenhuma", "A venda", 150.2, 30.5, new AuthorDTO(bob).getId());
		
		postRepository.saveAll(Arrays.asList(post1, post2, post3));
		
		Comment c1 = new Comment(null, "Post1Comment", sdf.parse("21/05/2019"), new AuthorDTO(alex).getId(), new PostDTO(post1).getId());
		Comment c2 = new Comment(null, "Post2Comment", sdf.parse("19/07/2020"), new AuthorDTO(bob).getId(), new PostDTO(post2).getId());
		Comment c3 = new Comment(null, "Post3Comment", sdf.parse("11/02/2021"), new AuthorDTO(maria).getId(), new PostDTO(post3).getId());
